...
describe('tarjanAlgorithmPure', () => {
  it('returns deterministic SCCs and low-link metadata', () => {
    const result = tarjanAlgorithmPure({
      0: ['1'],
      1: ['2', '3'],
      2: ['0'],
      3: ['4'],
    ...
    expect(result.indexById).toEqual({
      0: 0,
      1: 1,
      2: 2,
      3: 3,
    ...
    expect(result.lowLinkById).toEqual({
      0: 0,
      1: 0,
      2: 0,
      3: 3,
  ...
  it('handles self-loops as singleton SCCs', () => {
    ...
    expect(result.sccs).toEqual([['2'], ['1'], ['0']]);
  });

  it('handles a single vertex', () => {
    expect(tarjanAlgorithmPure({ 0: [] }).sccs).toEqual([['0']]);
  });
});

describe('tarjanAlgorithm visualization', () => {
  it('emits SCC discovery steps with badges and completed node states', () => {
    const steps = tarjanAlgorithm({
      nodes,
      edges,
      adjacency: {
        0: ['1'],
    ...
    expect(last.graphArtifacts.badges).toHaveLength(3);
